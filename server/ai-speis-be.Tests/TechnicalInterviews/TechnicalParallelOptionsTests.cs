...
public sealed class TechnicalEvaluationOptionsTests
{
    [Fact]
    public void FromConfiguration_ReadsEvaluationBudgetAndTokenPricing()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["TECHNICAL_AI_INPUT_TOKEN_COST_PER_MILLION"] = "0.20",
                ["TECHNICAL_AI_OUTPUT_TOKEN_COST_PER_MILLION"] = "0.80",
            })
            .Build();
        ...
        Assert.Equal(0.20m, options.InputTokenCostPerMillion);
        Assert.Equal(0.80m, options.OutputTokenCostPerMillion);
    }
}
