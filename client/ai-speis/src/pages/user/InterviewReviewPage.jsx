    ...
    body = (
      <div className="interview-review-layout">
        ...
        <article className="interview-review-detail">
          ...
          {selected.dimensions?.length ? <section className="interview-review-rubric"><h3><Target size={18} /> {copy.review.rubric}</h3><div>{selected.dimensions.map((dimension) => <article key={dimension.rubricCode || dimension.name}><div><strong>{dimension.name || dimension.rubricCode}</strong>{dimension.level ? <span>{dimension.level}</span> : null}</div>{formatScore(dimension.score, dimension.maxScore) ? <b>{formatScore(dimension.score, dimension.maxScore)}</b> : null}{dimension.evidence?.length ? <p>{dimension.evidence.join(' ')}</p> : null}</article>)}</div></section> : null}
