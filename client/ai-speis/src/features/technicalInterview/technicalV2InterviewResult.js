  ...
  return {
    ...
    questions: (result.mainQuestions || []).map((question, index) => {
      ...
      return {
        ...
        missingPoints: (question.missingPoints && question.missingPoints.length > 0) ? question.missingPoints : (summary.knowledgeGaps || summary.areasForImprovement || []),
        strengths: (question.strengths && question.strengths.length > 0) ? question.strengths : (summary.strengths || []),
        transcript: question.answerTranscript || '',
        feedbackSummary: summary.executiveSummary || summary.overallTechnicalAssessment || '',
        suggestions: summary.recommendationsForImprovement || summary.recommendations || [],
        subQuestions,
